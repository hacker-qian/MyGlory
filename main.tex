...
\subsubsection{Affinity between vCPU and Memory }
...
\subsubsection{Affinity between NIC buffer and Memory}
...
\subsubsection{Affinity between vCPU and NIC}
...
\begin{equation}
An=CPI_0/(\sum_{i=1}^N CPI_i)
\end{equation}

$A_n$ achieves best when all the VM vCPU execute on the node near to NIC, in this case $A_n=1$. When VM do not execute on the node 0, the $A_n$ achieve minimum and equal to 0.
...
\begin{equation}
f(A_m, A_n, \alpha, \beta)
\end{equation}

As for I/O intensive virtual machines, data interacting with the host machine is frequently. When the NIC receives a large amount of packets, interrupts raising from the NIC also handled by the vCPU of the VM, packets were copied to the memory buffers by using DMA. In this case, the asymmetric I/O accesses are main factors which influencing the performance, our model function focus on optimizing the affinity $A_n$. As for CPU intensive virtual machine, the vCPU threads frequently access memory to read and write data. In this case, the main factor resulting in the performance degradation is asymmetric memory access, our model function should focus on optimizing the affinity $A_m$.The optimal function can be instanced as follows:
\begin{equation}
f= max (\alpha A_n+ \beta A_m)
\end{equation}

where $\alpha$ is the weight of $A_n$ and ($\beta$) is the weight of $A_m$

But not the maximum affinity will achieve best performance, because maximum data locality will increase shared resource contention such as cache contention and affects performance. Examination of prior work [] shows the relationship between optimum performance and maximum affinity, when the references of affinity up to 80\%, the system performance achieve optimum, so our optimal function also refers to this value.
...
\section{Design}
In this section, we describe the detail design of our vNUIOA, a platform independent runtime system that optimizing the overheads of asymmetric access architecture. We enabling three parts for incorporating NUMA and NUIOA overheads awareness with hypervisor: namely, runtime performance monitor, topology aware information analyzer and dynamic load balance scheduler.

\subsection{Overview}
An overview of vNUIOA is shown in Figure 5. It consists of the following three parts: the \textbf{\emph{Runtime Performance Collector}} module collects and store the information of guest os since the system booting, then the \textbf{\emph{Topology aware Information Analyzer}} module analyzes and processes these information based on the topology of host machine. According to the analysis results, the \textbf{\emph{Dynamic Load Balance Scheduler}} module calculates the optimal affinity model(which describe in the section 3), migrating memory and remapping vCPUs with the consideration of load balance. The details of each module are described as follows.
...
\subsection{Runtime Performance Collector}
Runtime information collector mainly collects the information we used in the affinity classes and metrics. To periodically collect the performance information of virtual machines and host server, it is imperative to analyze memory and I/O access activities in the virtualized system. To end this, we use the Linux performance monitoring tool perfmon [] to detect memory and I/O access activities, then we store these information in the VMinfo table for each VM. Each table entry contains information about the accumulated number of VM access to each node vector($NC_n$), VM memory distribution vector($VMD_n$), VM vCPU CPI on each node vector($CPI_n$) and number of VM I/O requests(VIOR). The details of these information listed in the Table 2:
\begin{table}[h]
...
\hline
$CPI_n$ &
\tabincell{l}{$CPI_n$ vector describes the execution cycles of \\VM vCPU on node n}\\
\hline
VIOR  &
\tabincell{l}{VIOR records the number of I/O requests per\\ second for VM .}\\
\hline
\hline
...
\hline
$DL_{mn}$ &
\tabincell{l}{Access latency between NUMA node m and n.}\\
\hline
\end{tabular}
\end{table}

%\begin{itemize}dynamic
...
\subsection{Topology aware Information Analyzer}
Due to the nontransparent management of Virtual machine monitor, an accurate understanding of underlying topology is needed. We mainly analyse the runtime load per node and average access latency between each node,

Runtime CPU load is also important for performance of physical server. As we experimented in the section 3.2.1, physical machine system load balance mechanism will disturb the distribution of hardware resources. So in our design, we must analyse the real time load based on the topology of host machine, we mainly consider average CPU load of each NUMA node,the per-node CPU load \emph{$L_n$} can be calculated as follows:
\begin{equation}
L_n=(\sum_{i=1}^M L_i) / M
\end{equation}

Where M is the number of cores per NUMA node. We test that when the system load per node exceed the $80\%$, the system performance begin to goes down. Based on this experimental value, we define the threshold of $L_n$ is 80\%, if the system load per node up to $80\%$, the load of the node is high.

Because of the hop-distance result in the previous NUMA affinity modeling inaccurate, in this work, we use a more accurate system information for NUIOA affinity modeling. We measure the average access latency $ADL_{mn}$ between each NUMA node by using the DL table(\emph{$DL_{mn}$}). Based on the DL table, the average data transfer latency between each node can be accurate calculate as follows:
\begin{equation}
ADL^T_n=\frac{VMD_n}{\sum_{i=1}^N VMD_i}DL_{mn}
\end{equation}

Where N is the number of NUMA nodes and m is the node which VM vCPUs running on.

\subsection{Dynamic Load Balance Scheduler}
Because of the affinity unawareness of load balance mechanism affects performance in the virtualized environment, we should take affinity and load balance both into consideration. There are two methods to achieve load balancing:\emph{push migration and pull migration}. In push migration, scheduler periodically checks the load of each processor and migrate threads from the processor with high load to processors with low load if needed. Pull migration refers to when a processor becomes idle and tries to steal threads form busy one. Our load balance mechanism work closely to the push migration method. Algorithm 1 shows the mechanism of our load balance aware migration scheduler.
...
\textbf{Step 2}: Calculating the optimal placement for each VM. For different type of virtual machine, we assign different weight($\alpha, \beta$) to the affinity $A_m, A_n$, in this way, we can figure out optimal function. I/O intensive VMs always have frequent I/O operation, so the affinity $A_n$ are more important in the optimal function, and as a result, the weight of the $A_n$ is dominant and we apply the equation 5 which we discuss in the section 3. CPU intensive VMs always read and write data with memory, in this case, the proportion of affinity $A_m$ is higher in the optimal function and we should assign a proper weight, equation 6 are used in this case.

\textbf{Step 3}: According to the calculating results in the step 2, we make the decision of memory migration and vCPU mapping. we only migrate memory when maximum number of remote node memory access is double than the number of local memory access (\emph{max(NC)>2*NC[0])}, because frequent memory migration also bring high overheads. We test several value and found that when select 2 times than the number of local access, we can achieve max benefits. The vCPU mapping also negotiate with the load balance mechanism in the system, the load per cpu core can be calculated by $L_i=\frac{CPUtime_i}{Elapsedtime_i}$. If the average CPU load per node is higher than 80\%, we should not map any vCPU to this node and select another node for vCPU remapping. We remapping the VM m vCPUs from the original node n to node k instead of the node p is beneficial only if the average access latency to node k($$) is less than average access latency to node p. remember $L_k$ and $L_p$ both less than 80\%.

\textbf{Step 4}:After finishing the migration of VM memory and vCPUs, the scheduler continue schedule new coming VMs.

%(\emph{$\sum_{i=1}^M L_i / M > 80\% $})
\begin{algorithm}[h]
\caption{Load Balance aware Migration Algorithm}
\label{algo:balance}
\begin{algorithmic}[1]

\STATE \textbf{Variables}: \emph{VIOR}, $L_n$, $NC_n$
\FOR {each VM m}
\STATE /*Periodic look at \emph{VIOR} vector*/
\IF { $NIOR \geq 500$}
...
We implemented the vNUIOA prototype in the KVM virtualized platform. The holistic system is implemented as individual daemous in the KVM virtual machine monitor, We now describes the implementation details of our system.
...
\textbf{\emph{Using page faults measure memory accesses}}: The OS has a methods to measure if the pages has been accessed by the page fault, While the page is not set as present in the page table, the OS is noticed about the faulting virtual address, as well as the thread ID that caused the fault. By tracking page faults, it possible to detect which vCPU threads are accessing the memory page. We using Memory\ Page Faults with vCPU theads id in the Linux performance monitor tool \emph{perfmon} to sampling the access number to each node of the vCPU thread.
